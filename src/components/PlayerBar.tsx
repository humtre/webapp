...
import type { ShuffleMode } from '@/hooks/usePlayer';
...
// Likes shuffle — shuffle + small heart at center
const ShuffleLikes = () => (
  <svg viewBox="0 0 16 16" fill="currentColor" width={16} height={16}>
    {SHUFFLE_PATHS}
    <path d="M8 3.2l-1.8-2C5-0.2 3 .5 3 2.2c0 1.2 1 2.3 5 5 4-2.7 5-3.8 5-5C13 .5 11-0.2 9.8 1.2z" transform="translate(3,-.5) scale(0.5,0.6)"/>
  </svg>
);
// Random shuffle — just shuffle
const ShuffleRandom = () => (
  <svg viewBox="0 0 16 16" fill="currentColor" width={16} height={16}>
    {SHUFFLE_PATHS}
  </svg>
...
const HeartFill  = () => <svg viewBox="0 0 16 16" fill="currentColor" width={14} height={14}><path fillRule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z"/></svg>;
const HeartEmpty = () => <svg viewBox="0 0 16 16" fill="currentColor" width={14} height={14}><path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01L8 2.748zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143c.06.055.119.112.176.171a3.12 3.12 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15z"/></svg>;
...
export default function PlayerBar({
  track, trackIndex, isPlaying, currentTime, duration, volume, shuffleMode,
  likes, onLike, onUnlike, onTogglePlay, onNext, onPrev, onSeek, onVolumeChange, onCycleShuffleMode, onMenuToggle,
}: Props) {
  ...
  return (
    <footer className={styles.bar}>
      ...
      <div className={styles.left}>
        ...
        <button
          className={`${styles.btn} ${styles.likeBtn} ${likes > 0 ? styles.on : ''}`}
          onClick={onLike}
          onContextMenu={(e) => { e.preventDefault(); onUnlike(); }}
      ...
      <div className={styles.controls}>
        <button
          className={`${styles.btn} ${styles.shuffleBtn} ${shuffleMode !== 'sequential' ? styles.on : ''}`}
          onClick={onCycleShuffleMode}
          aria-label={`Shuffle: ${shuffleMode}`}
        >
          {shuffleMode === 'likes' ? <ShuffleLikes /> : <ShuffleRandom />}
        </button>
